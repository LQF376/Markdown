...
//* 将 logline 中的信息，存入到 buffer_ 中，若 buffer_ 存满，压入 vector 中待处理
//* 使用了双缓存保证，压入操作的同时，保证了数据的记录 currentBuffer_ 和 nextBuffer_
void AsyncLogger::Append(const char* logline, int len)
{
	...
	if (currentBuffer_->Available() > len) {
		...
		if (nextBuffer_) {			 //* 若下一个可用缓冲区不为空，则 move 给 cur
...
void AsyncLogger::ThreadFunc()
{
	...
	//* 消费线程核心逻辑：
	//* 	1. 上锁区域，将 BufferVector swap 出来，currentBuff_ 和 nextBuffer_ 重置；快进快出
	//* 	2. 对 BufferVector 中的消息进行处理，写进 LogFile 中
	//* 	3. 清空 BufferVector、newBuffer1、newBuffer2 用于下次交换
	while (is_running_) {
		{
			std::unique_lock<std::mutex> lock(mutex_);
